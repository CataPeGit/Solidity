// we will use state hook for approving, staking and unstaking
...
export const useStakeTokens = (tokenAddress: string) => {
    // in order to get the approve we need:
    // address , abi, chanId
    ...
    }, [approveAndStakeErc20State, amountToStake, tokenAddress])// if anything in the array changes we apply useEffect

    const [state, setState] = useState(approveAndStakeErc20State)

    // tracking both the state and the use of it
    useEffect(() => {
        if (approveAndStakeErc20State.status === "Success") {
            setState(stakeState)
        }
