...
def get_contract(contract_name):
    ...
    if network.show_active() in LOCAL_BLOCKCHAIN_ENVIROMENTS:
        ...
        contract_address = config["config"]["networks"][network.show_active()][
            contract_name
        ]
...
def deploy_mocks(decimals=DECIMALS, initial_value=INITIAL_VALUE):
    ...
    link_toke = LinkToken.deploy({"from": account})
    VRFCoordinatorMock.deploy(link_toke.address, {"from": account})
